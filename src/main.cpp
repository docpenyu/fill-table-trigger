...
int main(int argc, char** argv)
{
    ...
    size_t buffer_size = hitRateHz * rowOfTable * sizeof(hit_raw_data) * 2 * 0.001 + sizeof(header) +
    ...
    FillMultiRowTable fill_Mtable_25(rowOfTable, nhit);
    const int N = 20;
    //{
    ...
    //        std::chrono::steady_clock::time_point start, scan_over, sort_over, slide_over;
    ...
    //        scan.push_back(std::chrono::duration_cast<std::chrono::microseconds>(scan_over -
    //        start).count());
    //        sort.push_back(std::chrono::duration_cast<std::chrono::microseconds>(sort_over -
    //        scan_over).count());
    //        slide.push_back(std::chrono::duration_cast<std::chrono::microseconds>(slide_over -
    //        sort_over).count());
    //    }
    ...
    //    size_t avg_scan  = std::accumulate(scan.begin(), scan.end(), 0) / scan.size();
    //    size_t avg_slide = std::accumulate(slide.begin(), slide.end(), 0) / slide.size();
    //    std::cout << "fill-table-1ns alg: scan " << avg_scan << " us"
    //              << ", slide " << avg_slide << " us"
    //              << ", total_usage " << avg_scan + avg_slide << " us" << std::endl;
    //}
    {
